...
def exp(RANDOM_SEED, area_len, time_long, grid_granularity, time_granularity, TASK_NUMS, UV_NUMS, UAV_NUMS,
    ...
    if issubclass(algorithmClass, GreedyBaseAlgorithm) and no_sense_map:
        robot_algorithm = algorithmClass(sense_area.len, thetas=(1, 1, 0))
    else:
        robot_algorithm = algorithmClass(sense_area.len)
    # MASys
    ...
    for task in tasks:
        ...
        # print(task.TR)
    # real word
    real_word = RealWorld(len(Regions), CANNOT_SENSE_RETE, (1, 1, 1))
    # sim
    ...
    real_cov, real_dist = MASys1.actualCovAndDist()
    return real_cov, real_dist
